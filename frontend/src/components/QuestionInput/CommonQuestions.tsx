...
export default function CommonQuestions({
  ...
  type Category = 'Trash' | 'Fences' | 'Flags' | 'Meeting'
  ...
  const questions: Record<
    Category,
    {
      question: string
      answer: string
      subQuestions?: { question?: string; answer?: string; buttonText: string; url?: string }[]
    }[]
  > = {
    Trash: [
      {
        ...
        subQuestions: [
          ...
          },
          {
            ...
            answer:
              `There is no garbage, recycling, or yard waste collection on the following holidays:\n\n` +
              ...
              `Regular collection resumes on the next scheduled collection day if your collection date falls on one of these holidays.`,
            buttonText: 'Holidays'
          }
        ]
      }
    ...
    Fences: [
      {
        ...
        subQuestions: [
          ...
          },
          {
            ...
            answer:
              'Fences in Cheval West can be made out of the following materials:\n\n' +
              '• **Black or White Aluminum**\n\n' +
              '• **Black or White Wrought Iron**\n\n' +
              'Chain link fences, galvanized, vinyl coated, or otherwise treated chain link fences, as well as lattice (whether wood or PVC), are prohibited.',
            buttonText: 'Material'
          },
          {
    ...
    Flags: [
      {
        ...
        subQuestions: [
          {
            question: 'What types of flags can I install?',
            answer:
              'According to the Florida Statutes and community guidelines, you are allowed to display the following types of flags:\n\n' +
              '1. United States Flag: You may display one portable, removable United States flag in a respectful manner.\n\n' +
              '2. State of Florida Flag: The official flag of the State of Florida.\n\n' +
              '3. Military Flags: Flags representing the United States Army, Navy, Air Force, Marine Corps, Space Force, or Coast Guard.\n\n' +
  ...
  const handleButtonClick = (category: Category) => {
    ...
    handleCommonQuestion(firstQuestion.question, firstQuestion.answer)
  }
    ...
    <div className={styles.container}>
      {!selectedCategory ? (
        <div className={styles.buttonContainer}>
          <button className={styles.button} onClick={() => handleButtonClick('Trash')}>
            <span className={styles.buttonText}>Trash</span>
          </button>
          <button className={styles.button} onClick={() => handleButtonClick('Fences')}>
            <span className={styles.buttonText}>Fences</span>
          </button>
          <button className={styles.button} onClick={() => handleButtonClick('Flags')}>
            <span className={styles.buttonText}>Flags</span>
          </button>
          <button className={styles.button} onClick={() => handleButtonClick('Meeting')}>
            <span className={styles.buttonText}>Meeting</span>
          </button>
        </div>
      ) : (
        <div className={styles.buttonContainer}>
          {questions[selectedCategory][0].subQuestions?.map((q, index) =>
            q.url ? (
              <a key={index} href={q.url} className={styles.button} target="_blank" rel="noopener noreferrer">
                <span className={styles.buttonText}>
                  {q.buttonText} <Icon style={{ fontSize: 12 }} iconName="ArrowUpRight8" />
                </span>
              </a>
            ) : (
              <button
                ...
                onClick={() => handleCommonQuestion(q.question || '', q.answer || '')}>
                <span className={styles.buttonText}>{q.buttonText}</span>
              </button>
            )
          )}
        </div>
